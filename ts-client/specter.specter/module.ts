...
import { MsgRequestLoan } from "./types/specter/tx";
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgRequestLoan({ value }: msgRequestLoanParams): EncodeObject {
			try {
				return { typeUrl: "/specter.specter.MsgRequestLoan", value: MsgRequestLoan.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			SpecterSpecter: new SDKModule(test)
		},
		registry: msgTypes
  }
}
