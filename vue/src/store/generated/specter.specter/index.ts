import { Client, registry, MissingWalletError } from 'specter-client-ts'

import { Loan } from "specter-client-ts/specter.specter/types"
import { Params } from "specter-client-ts/specter.specter/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: specter.specter initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.SpecterSpecter.query.queryParams()).data
		...
		async QueryLoan({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SpecterSpecter.query.queryLoan( key.id)).data
		...
		async QueryLoanAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.SpecterSpecter.query.queryLoanAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.SpecterSpecter.query.queryLoanAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgRequestLoan({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.SpecterSpecter.tx.sendMsgRequestLoan({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgRequestLoan({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.SpecterSpecter.tx.msgRequestLoan({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgRequestLoan:Init Could not initialize signing client. Wallet is required.')
